...
boolean add_addr_update_request(unsigned int IC_addr, operand *operand, address_update_request_ptr *request_list) {
    ...
    if(new_addr_update_request == NULL) {
        safe_free((void**)&new_addr_update_request);
        return false;
    }
...
boolean process_addr_update_requests(assembler_context *asmContext) {
    ...
    while (address_update_request != NULL) {
        ...
        if (is_label_defined(label_name, asmContext->labels)) {
            ...
            print_external_error(ERROR_CODE_146,asmContext);
            return false;
        }
...
boolean relocate_data_memory(assembler_context *asm_context) {
    ...
    if (!asm_context) {print_internal_error(ERROR_CODE_25, "relocate_labels_addresses"); return false;}
...
boolean relocate_instructions_memory(assembler_context *asmContext) {
    ...
    if (!asmContext) {print_internal_error(ERROR_CODE_25, "relocate_instructions_memory"); return false;}

    temp = asmContext->instruction_memory;
    /*move the instruction memory after the memory offset address*/
    while(temp != NULL) {
