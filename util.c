 ...
 *        whitespace trimming, base-4 formatting, memory allocation wrapers, line typing,
 *        and global cleanup and many other utility functions.
 *
 * Provides small, reusable routines used across preprocessing, first/second pass,
 * encoding, files, and error handling modules.
 *
 * Dependencies: stdio.h, string.h, ctype.h, stdlib.h and project headers.
 *
...
void* handle_realloc(void* pointer,unsigned long size) {
    /*reallocate memory with the provided size*/
    void* p = realloc(pointer, size);
    if (!p) {
        /*print error if memory not reallocated*/
        print_system_error(ERROR_CODE_11);
        return NULL;
    }
    return p;
}
...
void free_all_memory(assembler_context *asmContext) {
    ...
    /*execute free function for all linked lists*/
